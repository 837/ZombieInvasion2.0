...
/**
 * provides functionality to -load shaderprograms -create FBO with textures Created by P on
 * 28.07.2015.
 */
public class GL_Helper {
  ...
  public static int generateFrameBufferObject() {
    ...
    if (!FBOEnabled)
      throw new UnsupportedOperationException(
          "cannot create fbo, please update Graphics Card drivers");

    IntBuffer buffer =
        ByteBuffer.allocateDirect(1 * 4).order(ByteOrder.nativeOrder()).asIntBuffer(); // allocate a
                                                                                       // 1 int byte
                                                                                       // buffer
  ...
  public static void renderToFBO(int FBO_ID, int width, int height) {
   ...
   // GL11.glBindTexture(GL_TEXTURE_2D, 0); // unlink textures because if we dont it all is gonna fail
    // glBindFramebufferEXT(EXTFramebufferObject.GL_FRAMEBUFFER_EXT, FBO_ID); // switch to rendering
    // on our FBO
   ...
   *         <p>
   *         shaderprogram can be used like this if(shaderLoadedCorrectly)
   *         ARBShaderObjects.glUseProgramObjectARB(program); shaders must be released after use
   *         if(shaderLoadedCorrectly) ARBShaderObjects.glUseProgramObjectARB(0);
   */
  public static int loadShaderProgram(String vertPath, String fragPath) {
    ...
    if (program == 0)
      throw new UnsupportedOperationException("couln't create program");

    /*
     * if the vertex and fragment shaders setup sucessfully, attach them to the shader program, link
     * the sahder program (into the GL context I suppose), and validate
     */
    ...
    if (ARBShaderObjects.glGetObjectParameteriARB(program,
      ...
      System.err.println(ARBShaderObjects.glGetInfoLogARB(program, ARBShaderObjects
          .glGetObjectParameteriARB(program, ARBShaderObjects.GL_OBJECT_INFO_LOG_LENGTH_ARB)));
  ...
  public static int generateTexture(int FBO_ID, int width, int height) {
   ...
   // EXTFramebufferObject.glBindFramebufferEXT(EXTFramebufferObject.GL_FRAMEBUFFER_EXT, FBO_ID);
    EXTFramebufferObject.glFramebufferTexture2DEXT(EXTFramebufferObject.GL_FRAMEBUFFER_EXT,
        EXTFramebufferObject.GL_COLOR_ATTACHMENT0_EXT, GL11.GL_TEXTURE_2D, colorTextureID, 0);
