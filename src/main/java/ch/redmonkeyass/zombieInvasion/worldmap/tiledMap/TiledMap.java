...
public class TiledMap {
   ...
   *        something else between lines (@see {@link #renderedLine(int, int, int)}
   * 
   *        TODO: [Isometric map] Render stuff between lines, concept of line differs from ortho
   *        maps
   */
    ...
    while (!allProcessed) {
      ...
      for (int burner = 0; burner <= min; currentTileX++, currentTileY--, burner++) {
        for (int layerIdx = 0; layerIdx < drawLayers.size(); layerIdx++) {
          ...
          currentLayer.render(currentLineX, initialLineY, currentTileX, currentTileY, 1, 0,
